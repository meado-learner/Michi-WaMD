...
export async function handler(chatUpdate) {
  ...
  try {
    ...
    try {
      ...
      let config = configCache[senderNumber]
      if (!config && await fs.promises.stat(configPath).catch(() => false)) {
        config = JSON.parse(await fs.promises.readFile(configPath))
        configCache[senderNumber] = config
      }
      if (config && config.prefix) {
        prefixRegex = config.prefix === 'multi'
          ? new RegExp('^[#$@*&?,;:+×!_\\-¿.]')
          : new RegExp('^(' + [...config.prefix].map(c =>
            c.replace(/([.*+?^${}()|\\])/g, '\\$1')
          ).join('|') + ')')
      }
    } catch (e) {
    ...
    // --- Inicialización de usuario/chat/settings optimizada ---
    try {
    ...
    // --- Sin delays ni setInterval innecesarios en la cola ---
    // Puedes implementar antispam/antiflood acá si quieres, pero no hagas delay a todos los mensajes

    if (m.isBaileys) return
    m.exp += Math.ceil(Math.random() * 10)
    let usedPrefix

    // --- Cacheo básico de metadata de grupo ---
    let groupMetadata = {}
    if (m.isGroup) {
      if (groupMetadataCache[m.chat]) {
        groupMetadata = groupMetadataCache[m.chat]
      } else {
        groupMetadata = await this.groupMetadata(m.chat).catch(_ => null) || {}
        groupMetadataCache[m.chat] = groupMetadata
        // Opcional: borra el cache tras 2 minutos
        setTimeout(() => delete groupMetadataCache[m.chat], 120000)
      }
      groupMetadata.participants = (groupMetadata.participants || []).map(p => ({ ...p, id: p.jid, jid: p.jid, lid: p.lid }))
    }

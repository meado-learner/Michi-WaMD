...
const handler = async (m, { conn, text, command }) => {
  try {
    ...
    const infoData = { title, canal, duracion, vistas, calidad, url };
    const thumbWithText = await drawThumbnailWithText(thumbnail, infoData);
...
async function drawThumbnailWithText(thumbnailUrl, info) {
  ...
  ctx.fillText(`🔗 ${info.url}`, 20, img.height - infoHeight + 140);
